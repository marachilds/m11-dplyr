...
str(pulitzer)
#Circulation numbers are characters because they have the comma for places
...
mutate(pulitzer, Pulitzer.Prize.Change = Pulitzer.Prize.Winners.and.Finalists..2004.2014 - Pulitzer.Prize.Winners.and.Finalists..1990.2003)

# What publication gained the most pulitzer prizes from 2004-2014?
# Be sure to use the pipe operator! 
most.prizes.2004.2014 <- filter(pulitzer, Pulitzer.Prize.Winners.and.Finalists..2004.2014 == max(Pulitzer.Prize.Winners.and.Finalists..2004.2014)) %>% select(Newspaper)
# started with: arrange(pulitzer, desc(Pulitzer.Prize.Winners.and.Finalists..2004.2014)) %>% 
...
biggest.decrease.with.at.least.5 <- select(pulitzer, Pulitzer.Prize.Winners.and.Finalists..2004.2014 >= 5) %>% 
